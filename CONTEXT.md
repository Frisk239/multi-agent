...
- **▶ 下一刀默认（roadmap §4）：** **G3-1 错误态三件套**（Wiki 页/RuntimesPage/记忆详情加 ErrorState+重试，热身刀）→ **G1-1 Pi runtime 真机验收 + RPC 命令面扩展**（closeout 钦定「最值得下一刀」）→ **G2-1 Deferred-escalation**（惰性升级，复用 `escalated_from_run_id`）。
