...
- **已推：** 真站体验队列含 G1/G6–G10/G12–G13/G15–G17 等（见 git log）  
- **本刀：** `agent-capability-tabs`（G13：概览/工作/能力/设置）  
- **closeout：** `app/.progress/agent-capability-tabs-impl-1.md`  
- **Multica 鉴权：** `app/.progress/multica-auth/`（storage-state gitignore）  
- **再下一刀建议：** `issue-subscribe`（G2）/ `issue-pr-link`（G3）/ `user-profile-brief`（G18）  
