...
> 路线/目标真源：[design/roadmap.md](../../design/roadmap.md)（G1–G5 Goal 体系 + §4 切片队列）。

```
人：/goal 定义目标（引用 roadmap §3 的 G1–G5；定北星与禁区）
   │
   ▼
Goal（G1 执行层诚实性 / G2 编排闭环 / G3 前端体验 / G4 知识记忆 / G5 可靠运营）
   │
   ▼
Slice Owner：从 roadmap §4 队列取刀 → 自动迭代（下方模型）→ 关刀时回写 §4 状态
```
