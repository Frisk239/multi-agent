...
- **▶ 待取刀：** 队列 33-40 已关（squad 归档/memory 项目上下文/历史小队/issue-due-date/automation-webhook×2/看板泳道/列表表格二阶，closeout 见 `app/.progress/*-impl-1.md`）——看板三视图+列表列选择/分组、webhook 全链路、due date 均就绪。调研池剩余：**webhook 频率限制**（G5）→ Memory/Wiki 知识反链（G4）→ 泳道跨道拖拽改派（G3）。G8-4b 仍禁开。
