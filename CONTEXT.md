...
- **▶ 待取刀：** `squad-retirement-dispatch-closure` 与 `memory-project-context` 均已关（队列 33/34，closeout 见 `app/.progress/*-impl-1.md`）：Squad DELETE=不可恢复归档+原子转交+派发 gate+归档只读；Memory 项目上下文 picker/三态缓存/名称回链/创建归属。G8-4b 仍禁开；下一刀候选见 memory-project-context closeout 尾部（Agent 环境变量编辑 / inline transcript 预览 / 历史小队入口）。
