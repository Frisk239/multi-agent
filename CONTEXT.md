...
- **▶ 待取刀：** `squad-retirement-dispatch-closure`、`memory-project-context`、`archived-squads-browsing` 均已关（队列 33/34/35，closeout 见 `app/.progress/*-impl-1.md`）：Squad 归档闭环 + 派发 gate + 归档只读；Memory 项目上下文（picker/三态缓存/名称回链/创建归属）；历史小队浏览 tab（服务端 ?archived= 三态补齐）。G8-4b 仍禁开；下一刀候选见 archived-squads-browsing closeout 尾部（inline transcript 预览 / 看板项目名投影 / 归档资源汇总）。
