...
- **▶ 待取刀：** 队列 33-39 已关（squad 归档/memory 项目上下文/历史小队/issue-due-date/automation-webhook×2/看板泳道，closeout 见 `app/.progress/*-impl-1.md`）——webhook 全链路 + 看板三视图（板/表/泳道）就绪。调研池剩余：**列表表格二阶**（G3）→ webhook 频率限制（G5）。G8-4b 仍禁开。
