...
- **已推：** 真站体验队列含 G1–G2/G6–G10/G12–G13/G15–G17 等（见 git log）  
- **本刀：** `issue-subscribe`（G2：关注/取消关注）  
- **closeout：** `app/.progress/issue-subscribe-impl-1.md`  
- **Multica 鉴权：** `app/.progress/multica-auth/`（storage-state gitignore）  
- **再下一刀建议：** `issue-pr-link`（G3）/ `user-profile-brief`（G18）/ `board-column-i18n`（G5）  
