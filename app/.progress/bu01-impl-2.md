...
- [x] Task 3–5 达标 — writer + 真表 API + 钩子 + Web + 回归 handoff  
- [x] 合成路径已删 — `routes/inbox.ts` 只读 `inbox_item`  
- [x] read/archive/角标可用 — API smoke + Sidebar `useInboxUnreadCount` 代码在位  
- [x] typecheck + 回归证据齐 — 计划者复验 typecheck 全绿；handoff smoke 采信  
- [x] 可开 PR — 新会话 code review，**勿 push main**  
- 偏离可接受：stale/orphan 也 notify；archive 顺带 read；server+web 同 commit  
- 人评：合并前建议本分支 `pnpm dev` 点侧栏角标 / Inbox 按钮（不挡 PR）  
- 结论：**整刀补1（Task 1–5）达标，允许开 PR 合 main**
