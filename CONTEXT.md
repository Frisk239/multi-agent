...
- **▶ 待取刀：** 队列 33-38 已关（squad 归档/memory 项目上下文/历史小队/issue-due-date/automation-webhook/webhook-payload-template，closeout 见 `app/.progress/*-impl-1.md`）——webhook 触发→payload 进模板供 agent 消费的全链路已闭环。调研池剩余：**看板泳道**（G3）→ 列表表格二阶（G3）→ webhook 频率限制（G5）。G8-4b 仍禁开。
