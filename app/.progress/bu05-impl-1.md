...
- [x] 0010 + unique — `uq_automation_run_rule_planned`；origin_rule_id  
- [x] run-now 建卡；二次 run-now 第二张卡 — handoff smoke  
- [x] tick 注册；非法 assignee failed — startAutomationWorker + insertFailedRun  
- [x] typecheck + smoke — 计划者复验 typecheck 全绿  
- 偏离可接受：UpdateAutomationRuleInput 用 base.partial；issues PUT 未抽 core  
