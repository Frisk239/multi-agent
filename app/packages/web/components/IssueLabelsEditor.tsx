...
/** Multica 对齐：默认只显示已挂芯片；展开才编辑/新建/归档 */
export function IssueLabelsEditor({ issue }: { issue: Issue }) {
  ...
  return (
    <div
      className={`issue-labels-editor${open ? ' issue-labels-editor--open' : ' issue-labels-editor--compact'}`}
      data-testid="issue-labels-editor"
    >
      <div className="issue-labels-editor-head">
        ...
        <div className="issue-labels-chips">
          {(issue.labels ?? []).length === 0 ? (
            <span className="text-dim text-sm">无</span>
          ) : (
            (issue.labels ?? []).map((l) => (
              <Link
                key={l.id}
        ...
        <button
          type="button"
          className="btn-ghost btn-sm issue-labels-edit-btn"
          data-testid="issue-labels-edit-toggle"
          aria-expanded={open}
          onClick={() => setOpen((v) => !v)}
        >
          {open ? '收起' : '编辑'}
