...
- **▶ 待取刀：** 队列 33-37 已关（squad 归档/memory 项目上下文/历史小队/issue-due-date/automation-webhook，closeout 见 `app/.progress/*-impl-1.md`）。调研池剩余：**webhook payload 模板注入**（`{{webhook.*}}`，续 37 底座）→ 看板泳道（G3）→ 列表表格二阶（G3）；Gantt 依赖 due date 已就绪。G8-4b 仍禁开。
