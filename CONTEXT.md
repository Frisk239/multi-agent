...
- **▶ 待取刀：** 队列 33-42 已关（squad 归档/memory 项目上下文/历史小队/issue-due-date/automation-webhook×3/看板泳道/列表表格二阶/webhook 限流/泳道跨道拖拽改派，closeout 见 `app/.progress/*-impl-1.md`）。调研池剩余：**Memory/Wiki 知识反链**（G4）→ Issue 卡片 inline due 编辑（G3）→ webhook deliveries 清理策略（G5）。G8-4b 仍禁开。
