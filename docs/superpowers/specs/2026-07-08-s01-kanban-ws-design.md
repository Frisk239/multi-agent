...
| ORM | Drizzle + better-sqlite3 | sqlc 风格类型安全；纯本地零配置（★1 实测：better-sqlite3 需 **^12.0.0**，11.x 无 Node 24 prebuilt）|
...
> **position 策略：** seed.js **没有 position 字段**。seed 脚本把 8 条 issue 的 position 全部设为 `0`，依赖 `ORDER BY position ASC, created_at DESC` 的二级排序（created_at DESC 兜底）。**seed 插入顺序 = 表中行序 = 期望列内显示顺序**（同一 status 的 issue，后插入的排后面）。seed 时按上表行序依次插入并逐条设 created_at 递增即可。新建的 issue 算 `MIN(position)-1 = -1` 浮顶，不影响 seed 的 0 值排序。
...
export const Assignee = z.object({
  type: AssigneeType,
  id: z.string().uuid(),
  label: z.string(),           // 展示用冗余字段："产品小队" / "产品·策划队长"；服务端 GET 时填充
}).nullable();
// null = 未指派；非 null 时 type+id+label 必须同时存在
```

> **为什么 assignee 带 label：** 前端卡片要显示 assignee 名称（§7.2），但 S01 只暴露 issue CRUD（§1.3），前端无法单独查 agent/squad 表。故 GET issues 时服务端用内存 map（静态 seed，O(1)）把 id 解析成 label，随 issue 一起返回。形态照 seed.js 的 `usedBy.label`。PUT/POST 输入不接受 label（服务端权威），仅输出携带。
...
// 单独导出 refine 校验，显式列出字段，避免 Object.keys 在 Zod 解析对象上的歧义
export const validateUpdateIssue = (d: UpdateIssueInput): boolean =>
  d.title !== undefined ||
...
// 用法：UpdateIssueInput.parse(body) 后调 validateUpdateIssue，false 则 400「至少传一个字段」
```
...
**WS 回声处理（发起窗口收到自己的事件）：**
发起拖拽的窗口 A 会收到自己触发的 `issue:updated` 广播（§6.6 规定含发起者）。窗口 A 已经做了乐观更新 + 收到 PUT 响应 reconcile，再收到 WS 事件是同一变更的二次通知。**WS 事件处理必须幂等**：用 issue `id` 在 React Query cache 里定位，若 cache 中该 issue 的状态已与事件一致则 no-op（React Query 的 `setQueryData` 天然幂等——写相同值不触发重渲染）。不要做"事件去重计数"或"忽略发起者"这类脆弱方案。

**cancelled 状态的可见性：**
§7.2 说 cancelled 不建列。但 `UpdateIssueInput.status` 允许 cancelled（PUT 接口不限制）。若某 issue 被改成 cancelled（正常 UI 无此入口，但 curl 直连可能触发），**前端渲染时 cancelled 的 issue 不进任何列**（filter 掉，不渲染卡片）。等 S02+ 加"隐藏已完成/已取消"筛选时正式处理。blocked 有专属列（§7.2），正常渲染。
...
| D11 | seed agent/squad id 用短标识符（`agt-lead`/`sqd-product`），但 schema 要求 `z.string().uuid()` | shared schema vs seed.js | seed.js 原始数据即短 ID | **S01 不阻塞**（前端不校验返回值，NewIssueForm 恒传 null）；S02+ 建议放宽 schema 为 `z.string()`（业务 id 用短标识符更合理），而非改 seed 成 UUID |
| D12 | 拖拽乐观更新（onMutate）未实现 | spec §7.3 ① | 计划代码 api.ts 与 KanbanBoard 接口矛盾（注释说在组件处理，组件没传 onMutate） | **S01 不阻塞**（WS 正常时 PUT ms 级视觉即时）；留 S02+ 实现 onMutate + onError 回滚。spec §9.3 "WS 断开卡片仍可拖"这条未达成 |

### 8.3 自审修订记录（2026-07-09 deep review）

spec 完成后做了一次批判性自审，发现并修复 5 个问题，记录于此供追溯：

| # | 问题 | 性质 | 修复 |
|---|---|---|---|
| R1 | `UpdateIssueInput.refine()` 用 `Object.keys(d).some(...)` 在 Zod 解析对象上有歧义，且错误信息挂整对象 | 代码质量 | §4.4 改为显式列出字段的 `validateUpdateIssue` 辅助函数，parse 后单独调 |
| R2 | assignee 只返回 `{type,id}` 无 label，前端卡片无法显示 assignee 名称，验收画面跑不通 | **契约断层（硬伤）** | §4.2 Assignee 加 `label` 字段，GET 时服务端用内存 map（静态 seed）填充 |
| R3 | seed 缺 position 值，浮顶排序初始态未定义 | 边界缺失 | §3.4 补 position 策略：seed 全设 0，依赖 created_at DESC 兜底，插入顺序=显示顺序 |
| R4 | 乐观更新 + WS 回声导致发起窗口二次处理同一变更 | 边界缺失 | §7.5 补：WS 事件处理必须幂等（setQueryData 幂等，不做去重计数） |
| R5 | cancelled 状态无列，PUT 允许改成 cancelled，前端行为未定义 | 边界缺失 | §7.5 补：cancelled 的 issue 渲染时 filter 掉不进任何列 |
