...
- **▶ 下一刀默认（roadmap §4）：** **G2-1 Deferred-escalation**（惰性升级，复用 `escalated_from_run_id`）→ G1-2 Grok ACP/fail-closed。已关：**G3-1 错误态三件套**（[closeout](app/.progress/g3-1-error-states-closeout-2026-08-02.md)）· **G1-1 Pi 真机验收 + RPC 命令面**（[closeout](app/.progress/g1-1-pi-real-closeout-2026-08-02.md)；pi 0.83 已装，steer/compact 运行中命令可用）。
