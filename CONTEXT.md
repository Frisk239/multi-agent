...
- **▶ 待取刀：** 队列 33-41 已关（squad 归档/memory 项目上下文/历史小队/issue-due-date/automation-webhook×3/看板泳道/列表表格二阶，closeout 见 `app/.progress/*-impl-1.md`）——webhook 全链路（触发→模板→限流）+ 看板三视图 + 列表二阶 + due date 均就绪。调研池剩余：**泳道跨道拖拽改派**（G3）→ Memory/Wiki 知识反链（G4）→ Issue 卡片 inline due 编辑（G3）。G8-4b 仍禁开。
