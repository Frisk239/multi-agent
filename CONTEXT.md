...
- **▶ 待取刀：** 队列 33-36 已关（squad 归档/memory 项目上下文/历史小队/issue-due-date，closeout 见 `app/.progress/*-impl-1.md`）。Multica 对照缺口扫描产出新候选池：**webhook 触发 Automation**（G5/G2，`autopilot_webhook.go:343`）→ 看板泳道（G3）→ 列表表格二阶（G3）；Gantt 依赖 due date 已就绪。G8-4b 仍禁开。
